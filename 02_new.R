# This file contains MAIC and STC functions based on Remiro et al. 
...
# results in a nested loop plot (R?cker, G., Schwarzer, G. Presenting simulation 
...
gradfn<-function(beta.pars,X){
  colSums(sweep(X, 1, exp(X %*% beta.pars), "*"))
}

# function to estimate MAIC weights (assumes M>1)

maic2 <- function(A.X, B.summary,sd.summary,data_IPD,data_ALD) {
  
  M<-ncol(A.X)
  # Check if M is greater than 1
  # if (!(M > 1)) {
  #   rlang::abort("M must be greater than 1")
  # }
  # # Loop through columns
  # for (i in 1:M) {
  #   # Print structure and number of columns in B.summary for debugging
  #   print("Debug: Structure of B.summary")
  #   print(str(B.summary))
  #   num_cols <- ncol(B.summary)
  #   print(paste("Number of columns in B.summary:", num_cols))
  #   
  #   # Check if B.summary has at least 11 columns to safely access columns 5 to 11
  #   if (num_cols >= 11) {
  #     sd.summary <- B.summary[, c(5:11)]
  #   } else if (num_cols >= 5) {
  #     sd.summary <- B.summary[, c(5:num_cols)]
  #     print("Selected available columns from 5 to", num_cols)
  #   } else {
  #     rlang::abort("B.summary does not have enough columns. Cannot select columns 5 to 11.")
  #   }
  #   
  #   # Safely access and perform operations with the selected columns
  #   if (i <= num_cols) {
  #     A.X[, i] <- sweep(with(IPD.AC2, cbind(A.X[, i], (A.X[, i])^2)), 2, 
  #                       with(ALD.BC2, c(B.summary[, i], B.summary[, i]^2 + sd.summary[, i]^2)), '-')
  #   } else {
  #     print(paste("Skipping index", i, "as it exceeds the number of columns in B.summary."))
  #   }
  # }
  # 
  # 
  ...
  # M <- ncol(A.X)
  if (!(M > 1)) {rlang::abort("M must be greater than 1")}
  for (i in 1:M) {
    # A.X[, i] <- A.X[, i] - B.summary[, i] # only means are balanced
    #===========================================================================
    #Here SEs could be balanced in the future?
    #ADD as a scenario?
    #============================================================================

    #Here SEs and mean are balanced

    A.X[,i] <- sweep(with(data_IPD , cbind(A.X[,i], (A.X[,i])^2)), 2,#ipd data
                       with(data_ALD, c(B.summary[,i], B.summary[,i]^2 + sd.summary[,i]^2)), '-')}
    #agd data

  A.X <- as.matrix(A.X)
  
  # arbitrary starting point for the optimiser
  beta.start <- rep(1, ncol(A.X))
  # optimisation (BFGS)
  out.optim <- optim(fn = objective.function,gr=gradfn, X = A.X, par = beta.start, method = "BFGS")
  # betas
  ...
  w <- exp(temp.w)
  return(w)
  #At the end, we got the estimated weights 
}
...
biaspersim <- function(theta.hat, theta) {
  #nsim <- length(theta.hat)
  est <- theta.hat - theta
  return(est)
}
...
#
# These are by: R?cker, G., Schwarzer, G. Presenting simulation results in a nested loop plot. 
