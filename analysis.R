...
for (i in 1:scenarios) {
  ...
  maic.no.wei.pro <- !is.na(means) & !is.na(variances) & variances<1000 #change here
  ...
  # number of replicates for which MAIC weighted regression has separability issues 
  maic.separability.list2[i] <- sum(abs(means2)>max.effect)
  # discard these replicates (1 of 162000 in this sim. study)
  maic.no.wei.pro2 <- !is.na(means2) & !is.na(variances2) & variances2<10 #change here
...
#scenarios.dfcond <- read.csv(file="Analysis/scenarios/cond.csv", header=TRUE, sep=",")
...
lines(pd.ese$maic.empse2, col="orange", lty=3, type="s", lwd=2.5) 
...
lines(pd.ese$maic.empse2, col="orange", lty=3, type="s", lwd=2.5) 
legend("bottom", lwd=c(2.5,2.5,2.5,2.5), col=c("red","green","blue","orange"), lty=c(2,5,4,3), 
       cex=1.15, bty="n", c("MAIC", "STC", "Bucher","MIAC2"))
dev.off()
...
lines(pd.cover$maic.cov2*100, col="orange", lty=3, type="s", lwd=2.5) 
...
lines(pd.cover$maic.cov2*100, col="orange", lty=3, type="s", lwd=2.5) 
legend("right", lwd=c(2.5,2.5,2.5,2.5), col=c("red","green", "blue","orange"), 
       lty=c(2,5,4,3), cex=1.15, bty="n",c("MAIC", "STC", "Bucher","MIAC2"))
dev.off()
...
lines(pd.bias$bucher.bias, col="blue", lty=4, type="s", lwd=2.5)
lines(pd.bias$maic.bias2, col="orange", lty=3, type="s", lwd=2.5)
legend("bottom",lwd=c(2.5,2.5,2.5),col=c("red","green", "blue","orange"),
       lty=c(2,5,4,3),cex=1.15,bty="n", c("MAIC", "STC", "Bucher", "MLNMR","MAIC2")) # legend
dev.off()
...
lines(pd.mse$maic.mae2, col="orange", lty=3, type="s", lwd=2.5)
legend("left",lwd=c(2.5,2.5,2.5),col=c("red","green","blue","orange"),lty=c(2,5,4,3),
       cex=1.15,bty="n",c("MAIC", "STC", "Bucher", "MLNMR","MAIC2"))
dev.off()
...
lines(pd.bias$maic.std.bias2, col="orange", lty=3, type="s", lwd=2.5)
legend("bottomleft",lwd=c(2.5,2.5,2.5), col=c("red","green","blue","orange"), lty=c(2,5,4,3),
       cex=1.15, bty="n", c("MAIC", "STC", "Bucher","MAIC2"))
dev.off()
...
lines(pd.mse$maic.uci.mean2-pd.mse$maic.lci.mean2, col="orange", lty=3, type="s", lwd=2.5)
legend("bottom", lwd=c(2.5,2.5,2.5), col=c("red","green","blue","orange"), lty=c(2,5,4,3),
       cex=1.15, bty="n", c("MAIC", "STC", "Bucher","MAIC2"))
dev.off()
...
lines(pd.cover$maic.error.worse.than2*100, col="orange", lty=5, type="s", lwd=2.5)
legend("bottom", lwd=c(2.5, 2.5), col=c("red","green","orange"), lty=c(2,5),
...
lines(maic.rolling.bias.2, col="orange", lwd=2, lty=3)
legend("bottom", legend = c("MAIC", "STC", "Bucher","MAIC2"),
       col = c("red", "green","blue","orange"), bty = "n",
       pt.cex = 2, cex = 1.2, text.col = "black", horiz = F , inset = c(0.1, 0.1),
       lwd=c(2,2,2,2), lty=c(2,5,4,3))
dev.off()